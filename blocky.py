...
class MainState(GameState):
    ...
    def process_event(self, event: pygame.event.Event) -> None:
        """calls process_event for player self._current_player with argument
        <event>"""
        self._current_player().process_event(event)

    def update(self) -> GameState:
        """ends the game iff the self._turn >= self._data.max_turns and returns
        the end of game score
        otherwise it generates a move for player self._current_player
        with the board
        and if the move is None it returns self
        otherwise if self._do_move with parameter move is True it will
        return the the animation of the move
        otherwise it will return self"""
    ...
    def render(self, renderer: Renderer) -> None:
        """draws the board of self._data.board
        if the board exists it is then highlighted
        draws the turn number, the current_player id, the current score
        and the goal descirption of the current player
         """
...
class AnimateMoveState(GameState):
    ...
    def update(self) -> GameState:
        """iff the animation is complete
        will return self._parent
    ...
    def render(self, renderer: Renderer) -> None:
        """draws board self._background
        draws an outline of the black in self._move
        draws the image representing the move in self._move
        a message is displayed that self._player is
        currently making the move"""
